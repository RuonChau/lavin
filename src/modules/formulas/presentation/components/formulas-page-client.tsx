...
import { FormulaDetailsModal } from '@/modules/formulas/presentation/components/modal/formula-details.modal';
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-[28px] font-bold text-text-primary tracking-tight">Quản lý Công thức / BOM</h1>
          <p className="text-text-muted text-sm mt-1 leading-relaxed">Thiết lập định mức nguyên liệu & tính toán giá thành sản phẩm chính xác.</p>
        </div>

        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 rounded-2xl bg-white/60 border border-primary-soft/30 px-5 py-3 text-sm font-bold text-text-secondary transition-all hover:bg-white hover:shadow-md">
            <FileDown size={18} />
            Xuất báo cáo
          </button>
          <button
            onClick={() => setIsAddModalOpen(true)}
            className="flex items-center gap-2 rounded-2xl bg-primary px-6 py-3 text-sm font-bold text-white shadow-lg shadow-primary/20 transition-all hover:bg-primary-deep hover:-translate-y-0.5"
          >
            <Plus size={20} />
            Thêm công thức
          </button>
      ...
      <GlassCard className="p-0 overflow-hidden" radius="4xl">
        ...
        <div className="p-6 border-b border-primary-soft/10 bg-white/40 flex flex-col sm:flex-row gap-4 items-center">
          <div className="relative flex-1 group">
            ...
            <input
              ...
              className="w-full pl-11 pr-5 py-3 rounded-2xl bg-[#FFFAF4]/60 border border-primary-soft/30 text-sm focus:outline-none focus:ring-4 focus:ring-primary/10 transition-all placeholder:text-text-muted"
            />
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-3 bg-[#FFFAF4]/60 border border-primary-soft/30 rounded-2xl px-4 py-3">
              <span className="text-xs font-bold text-text-muted uppercase tracking-widest whitespace-nowrap">Danh mục:</span>
              <select className="bg-transparent text-sm font-bold text-text-secondary focus:outline-none cursor-pointer pr-4">
                <option value="all">Tất cả</option>
                <option value="coffee">Cà phê</option>
                <option value="tea">Trà & Đá xay</option>
              </select>
            </div>
            <button className="p-3 bg-[#FFFAF4]/60 border border-primary-soft/30 rounded-2xl text-text-secondary hover:bg-white transition-all shadow-sm">
              <Filter size={18} />
            </button>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            <div className="py-24 flex flex-col items-center justify-center">
              ...
              <p className="text-sm font-bold text-text-muted">Chuẩn bị dữ liệu công thức...</p>
            </div>
          ) : (
                  ...
                  >
                    <td className="py-5 px-8">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl overflow-hidden border border-primary-soft/20 shadow-sm transition-transform group-hover:scale-110">
                          <img
                        ...
                        <div>
                          <p className="text-sm font-bold text-text-primary leading-tight">{formula.productName}</p>
                          <p className="text-[10px] text-text-muted mt-1 font-bold bg-white/60 px-2 py-0.5 rounded-md border border-primary-soft/10 inline-block">
                    ...
                    <td className="py-5 px-6 text-center">
                      <p className="text-sm font-black text-text-primary">₫{formula.totalCost.toLocaleString('vi-VN')}</p>
                      <p className="text-[9px] text-green-600 font-bold uppercase tracking-tighter">Tối ưu</p>
                    </td>
                    <td className="py-5 px-6">
                      <p className="text-xs font-bold text-text-secondary">{new Date(formula.updatedAt).toLocaleDateString('vi-VN')}</p>
                      <p className="text-[10px] text-text-muted">Bởi Admin</p>
                    </td>
        ...
        {!isLoading && filteredFormulas.length === 0 && (
          <div className="py-24 flex flex-col items-center justify-center">
            <div className="w-20 h-20 rounded-4xl bg-[#FFFAF4] flex items-center justify-center text-primary-soft mb-4 border border-primary-soft/20">
              <AlertCircle size={32} />
            </div>
            <p className="text-base font-bold text-text-primary">Không tìm thấy công thức !</p>
            <p className="text-sm text-text-muted mt-1">Vui lòng kiểm tra lại từ khóa tìm kiếm hoặc bộ lọc.</p>
        ...
        {/* Pagination Info */}
        <div className="p-6 border-t border-primary-soft/10 bg-[#FFFAF4]/20 flex items-center justify-between">
          <p className="text-[11px] font-bold text-text-muted uppercase tracking-widest">Hiển thị {filteredFormulas.length} trên {formulas.length} công thức</p>
          <div className="flex gap-2">
            <button disabled className="w-9 h-9 rounded-xl border border-primary-soft/20 flex items-center justify-center text-text-muted disabled:opacity-30">
            ...
            <button className="w-9 h-9 rounded-xl border border-primary-soft/20 flex items-center justify-center text-text-muted hover:bg-white transition-all">
              <Filter className="-rotate-90" size={14} />
            </button>
          </div>
        </div>
