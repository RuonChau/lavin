    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <button className="flex items-center gap-2 px-5 py-3 bg-white border border-primary-soft/30 rounded-2xl text-[10px] font-black text-text-secondary uppercase tracking-wider hover:bg-gray-50">
      ...
      <GlassCard className="overflow-hidden" radius="4xl">
          ...
          <thead>
            <tr className="bg-[#FFFAF4]/40 border-b border-primary-soft/20">
              <th className="py-6 px-8 text-left">
                <span className="text-[10px] font-black text-text-muted uppercase tracking-[0.2em]">Nhân viên / Ca</span>
              </th>
              <th className="py-6 px-4 text-center">
                <span className="text-[10px] font-black text-text-muted uppercase tracking-[0.2em]">Giờ quy chuẩn</span>
              </th>
              <th className="py-6 px-4 text-center">
              ...
              <tr key={record.id} className="group hover:bg-[#FFFAF4]/20 transition-colors">
                <td className="py-5 px-8">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-sm font-bold text-text-primary">{record.name}</p>
                      <p className="text-[9px] font-black text-primary uppercase tracking-widest mt-0.5">Mã ca: {record.shift}</p>
                    </div>
                  </div>
                </td>
                <td className="py-5 px-4 text-center">
                  <span className="text-xs font-bold text-text-secondary">07:00 - 15:00</span>
                </td>
                <td className="py-5 px-4 text-center">
                  <span className={cn(
                    "text-xs font-black",
                    record.checkIn === '---' ? "text-primary-soft/50" : "text-text-primary"
                  )}>
                    {record.checkIn}
                  </span>
                </td>
                <td className="py-5 px-4 text-center">
                  <span className={cn(
                    "text-xs font-black",
                    record.checkOut === '---' ? "text-primary-soft/50" : "text-text-primary"
                  )}>
                    {record.checkOut}
                  </span>
                </td>
                <td className="py-5 px-4 text-center">
                  {record.late > 0 ? (
                    ...
                    <span className="text-xs text-primary-soft/40 font-bold">-</span>
                  )}
                </td>
                <td className="py-5 px-4 text-center">
        ...
        <div className="p-8 border-t border-primary-soft/20 bg-white/40 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button className="p-2 text-text-muted hover:bg-white rounded-xl transition-all border border-transparent hover:border-primary-soft/20"><ChevronLeft size={18} /></button>
            <div className="flex items-center gap-1">
              <button className="w-8 h-8 rounded-xl bg-primary text-white text-[10px] font-black">1</button>
              <button className="w-8 h-8 rounded-xl bg-white text-text-muted text-[10px] font-black border border-primary-soft/20">2</button>
              <button className="w-8 h-8 rounded-xl bg-white text-text-muted text-[10px] font-black border border-primary-soft/20">3</button>
            </div>
            <button className="p-2 text-text-muted hover:bg-white rounded-xl transition-all border border-transparent hover:border-primary-soft/20"><ChevronRight size={18} /></button>
          </div>
      ...
      >
        ...
        >
          ...
          >
            <Input.TextArea rows={3} placeholder="Ví dụ: Quên điện thoại không check-in được" className="rounded-xl bg-white/60 border-primary-soft/30 hover:border-primary/50 focus:border-primary/50 shadow-sm py-3" />
          </Form.Item>
