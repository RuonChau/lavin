    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="flex bg-white/60 border border-primary-soft/20 rounded-2xl p-1 shadow-sm items-center relative group">
            ...
            <div className="relative flex flex-col items-center justify-center min-w-45 h-10 w-full overflow-hidden cursor-pointer hover:bg-white/40 transition-colors rounded-xl">
              ...
              <span className="text-xs font-black text-text-primary leading-none">15/05 - 21/05</span>
            </div>
            <button
              onClick={() => setCurrentWeek(w => w + 1)}
              className="p-2 text-text-muted hover:text-primary hover:bg-[#FFFAF4] rounded-xl transition-all z-10"
        ...
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 px-6 py-3 bg-white border border-primary-soft/30 rounded-xl text-[10px] font-black text-text-secondary uppercase tracking-wider hover:bg-gray-50 transition-all active:scale-95 shadow-sm">
            Sao chép tuần trước
          </button>
      ...
      <Modal
        title={<span className="text-xl font-black text-text-primary tracking-tight italic">Chi tiết Nghỉ phép</span>}
        open={!!selectedLeave}
        ...
        {selectedLeave && (
          <div className="mt-6 space-y-6">
            <div className="flex items-center gap-4 p-4 rounded-2xl bg-white/60 border border-primary-soft/20 shadow-sm">
              ...
              <div>
                <p className="text-lg font-black text-text-primary italic">{selectedLeave.empName}</p>
                <p className="text-[10px] font-black text-primary uppercase tracking-widest mt-0.5">{selectedLeave.date}</p>
              </div>
            </div>

            <div className="space-y-4">
              <div className="bg-white/60 p-4 rounded-2xl border border-primary-soft/20 shadow-sm">
                <p className="text-[10px] font-black text-primary-soft uppercase tracking-[0.2em] mb-2 flex items-center gap-2">
                  <AlertCircle size={14} className="text-amber-500" />
                  Lý do nghỉ
                </p>
                <p className="text-sm font-bold text-text-primary">{selectedLeave.reason}</p>
              </div>

              {selectedLeave.swapWith && (
                <div className="bg-[#FFFAF4]/60 p-4 rounded-2xl border border-primary-soft/20 shadow-sm">
                  <p className="text-[10px] font-black text-primary-soft uppercase tracking-[0.2em] mb-3 flex items-center gap-2">
                    <Users size={14} className="text-primary" />
                    Nhân sự thay thế (Đổi ca)
                  </p>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-xl bg-white border border-primary-soft/20 flex items-center justify-center text-primary font-black text-xs shadow-sm">
                        {selectedLeave.swapWith.split(' ').pop()?.[0]}
                      </div>
                      <span className="text-sm font-bold text-text-primary">{selectedLeave.swapWith}</span>
                    </div>
                    <span className="px-3 py-1 bg-primary/10 text-primary border border-primary-soft/20 rounded-lg text-[9px] font-black uppercase">Đã xác nhận</span>
                  </div>
                </div>
              )}
            </div>
