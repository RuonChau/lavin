    ...
    },
    {
      ...
      render: (salary) => (
        canManage ? (
          ...
          <span className="text-xs font-bold text-text-muted tracking-widest">••••••••</span>
        )
      ),
    },
    {
          ...
          {canManage ? (
            <Tooltip title={record.status === 'ACTIVE' ? 'Nhấn để chuyển sang Đã nghỉ' : 'Nhấn để chuyển sang Đang làm việc'}>
              <button
                ...
                className={cn(
                  "inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-wider border transition-opacity hover:opacity-80",
