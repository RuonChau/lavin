      ...
      <GlassCard className="relative p-8" radius="4xl">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <h2 className="text-xl font-bold text-text-primary">Điều chỉnh tồn kho</h2>
        ...
        <div className="space-y-6">
          {/* Material Info */}
          <div className="p-4 rounded-2xl bg-[#FFFAF4] border border-primary-soft/20 flex justify-between items-center">
            <div>
              <p className="text-xs font-bold text-text-muted uppercase">Nguyên liệu</p>
              <p className="text-sm font-bold text-text-primary">{material.name}</p>
            </div>
            <div className="text-right">
              <p className="text-xs font-bold text-text-muted uppercase">Hiện tại</p>
            ...
            <button
              ...
              className={cn(
                ...
                type === 'INCREASE'
                  ...
                  : "bg-white/40 border-primary-soft/20 text-text-muted"
              )}
            >
          ...
          <div className="space-y-4">
            <div>
              <label className="text-[10px] font-bold text-text-muted uppercase tracking-widest block mb-2 px-1">Số lượng thay đổi ({material.unit})</label>
              <input
                type="number"
                value={adjustment}
                onChange={(e) => setAdjustment(Math.max(0, Number(e.target.value)))}
                className="w-full bg-white rounded-xl border border-primary-soft/20 px-4 py-3 text-lg font-black text-text-primary focus:outline-none focus:border-primary shadow-inner"
              />
            </div>

            <div>
              <label className="text-[10px] font-bold text-text-muted uppercase tracking-widest block mb-2 px-1">Lý do điều chỉnh</label>
              <textarea
                value={reason}
                onChange={(e) => setReason(e.target.value)}
                placeholder="VD: Kiểm kho định kỳ, Hàng lỗi..."
                className="w-full bg-white rounded-xl border border-primary-soft/20 px-4 py-3 text-sm font-medium text-text-primary focus:outline-none focus:border-primary shadow-inner min-h-[80px]"
              />
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-primary/5 border border-primary/20 flex justify-between items-center">
            <span className="text-sm font-bold text-text-secondary">Tồn kho dự kiến:</span>
          ...
          <div className="flex gap-3 pt-2">
            <button
              onClick={onClose}
              className="flex-1 py-3 rounded-2xl bg-white border border-primary-soft/30 text-sm font-bold text-text-secondary transition-all active:scale-95"
            >
              Hủy
            </button>
            <button
