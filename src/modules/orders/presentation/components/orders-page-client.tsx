...
import { ConfirmPaymentModal } from '@/modules/orders/presentation/components/modal/confirm-payment.modal';
import { StatusUpdateModal } from '@/modules/orders/presentation/components/modal/status-update.modal';
import { CancelOrderModal } from '@/modules/orders/presentation/components/modal/cancel-order.modal';
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-text-primary tracking-tight">Quản lý Đơn hàng</h1>
          <p className="text-text-secondary mt-1 text-sm">Theo dõi, xử lý và cập nhật trạng thái đơn hàng thời gian thực.</p>
        </div>
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-white border border-primary-soft/30 text-sm font-bold text-text-secondary shadow-sm hover:shadow-md transition-all">
            <Download size={18} />
            Xuất báo cáo
          </button>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 bg-white border border-primary-soft/20 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all text-text-primary placeholder:text-text-secondary"
            />
          </div>
          <div className="flex items-center gap-3">
            ...
            <button className="p-3 bg-white border border-primary-soft/20 rounded-2xl text-text-secondary hover:bg-white/60 transition-all">
              <Filter size={20} />
            </button>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white/60 border-b border-primary-soft/10">
                ...
                <th className="py-4 px-6 text-[11px] font-bold text-text-muted uppercase tracking-widest">Thanh toán</th>
                <th className="py-4 px-6 text-[11px] font-bold text-text-muted uppercase tracking-widest">Trạng thái</th>
                <th className="py-4 px-6 text-[11px] font-bold text-text-muted uppercase tracking-widest text-right">Thao tác</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                ...
                return (
                  <motion.tr
                    ...
                    animate={{ opacity: 1 }}
                    key={order.id}
                    className="border-b border-primary-soft/5 hover:bg-white/30 transition-all group"
                  >
                    <td className="py-4 px-6">
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-text-primary group-hover:text-primary transition-colors">{order.orderNumber}</span>
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-white/80 flex items-center justify-center text-primary">
                          <User size={14} />
                        </div>
                        <span className="text-sm font-semibold text-text-primary">{order.customerName || 'Khách vãng lai'}</span>
                      </div>
                    </td>
                    <td className="py-4 px-6">
                      <span className="text-[12px] font-mono text-text-secondary flex items-center gap-1.5">
                        <Calendar size={12} className="text-text-secondary" />
                        {new Date(order.createdAt).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}
                      </span>
                    </td>
                    <td className="py-4 px-6">
                      <span className="text-sm font-bold text-text-primary">₫{order.totalAmount.toLocaleString('vi-VN')}</span>
                    </td>
                    <td className="py-4 px-6">
                      <div className="flex flex-col">
                        <span className={cn("text-[11px] font-bold", payment.color)}>{payment.label}</span>
                        <span className="text-[10px] text-text-secondary">{order.paymentMethod}</span>
                      </div>
                    </td>
                    <td className="py-4 px-6">
                      <div className={cn(
      ...
      {openMenuId && (() => {
        ...
        return (
          <div
            ...
            className="w-64 bg-white rounded-4xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-primary-soft/5 py-4 text-left"
          >
            <div className="px-4 pb-2 mb-2 border-b border-primary-soft/5">
              <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest">Xử lý nhanh</p>
            </div>
            <button
              onClick={() => { handleActionClick(order, 'status'); setOpenMenuId(null); }}
              className="w-full text-left px-6 py-3 text-sm font-bold text-text-secondary hover:bg-primary-soft/10 hover:text-primary flex items-center gap-3 transition-colors"
            >
            ...
            <div className="my-2 border-t border-primary-soft/5 mx-4" />
            <button
